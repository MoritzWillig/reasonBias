...
def main():
    ...
    with (eval_path / "data_dump.json").open("w") as f:
        json.dump(data_dump, f, ensure_ascii=True, indent=2)
    with (eval_path / "data_dump_skipped_answers.json").open("w") as f:
        json.dump(all_skipped_answers, f, ensure_ascii=True, indent=2)
    with (eval_path / "data_dump_response_counts.json").open("w") as f:
